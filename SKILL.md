...
1. `MyWorkoutProgram-Routine.csv` and `MyWorkoutProgram-MuscleGroupSetsPerWeek.csv`: Ground truth for split, exercises, and volume. Do not modify unless asked.
...
1. Review MyWorkoutProgram CSVs, calendar history, and readiness metrics.
...
**Source of Truth:** Refer to `MyWorkoutProgram-Routine.csv` and `MyWorkoutProgram-MuscleGroupSetsPerWeek.csv` for all volume targets and weekly split.
